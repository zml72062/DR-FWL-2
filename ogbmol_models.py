...
from pygmmpp.datasets import OGBG
...
def epoch(model, 
    ...
    for batch in loader:
        ...
        batch_loss = nn.BCEWithLogitsLoss()(pred[is_labeled], 
                                          y[is_labeled])
        if optimizer is not None:
            batch_loss.backward()
            optimizer.step()
...
parser.add_argument('--config-path', type=str, default='configs/ogbmol.json',
...
def train_on_ogb(seed):
    ...
    """
    scheduler = StepLR(optimizer, loader.train.lr_step_size, 
                       loader.train.lr_reduce_factor)
    ...
    return run(loader.train.epochs,
              ...
              val_dataset,
              test_dataset,
              Evaluator(loader.dataset.name),
              metric_names[loader.dataset.name],
              device,
              optimizer,
              scheduler,
              'max')

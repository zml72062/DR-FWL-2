...
class GNN(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers, concat=False, y_ndim=1, **kwargs):
        ...
        M_in, M_out = 8, hidden_channels
        self.convs.append(GatedGraphConv(M_out, 1))


        for i in range(num_layers-1):
            M_in, M_out = M_out, hidden_channels
            self.convs.append(GatedGraphConv(M_out, 1))

        fc_in += M_out
        if self.concat:
            self.fc1 = torch.nn.Linear(32 + hidden_channels*(num_layers-1), 32)
        else:
    ...
    def forward(self, data):
        ...
        # if self.y_ndim == 1:
        #     x = scatter_add(x, data.batch0, dim=0)
...
class I2GNN(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers, subgraph_pooling='mean', use_pos=False,
        ...
        self.convs = torch.nn.ModuleList()
        M_in, M_out = hidden_channels, hidden_channels
...
class PPGN(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers, use_embedding=False, use_spd=False, y_ndim=1,
                 **kwargs):
        ...
        # ppgn modules
        num_blocks = 2
        num_rb_layers = num_layers
        num_fc_layers = 2

        self.ppgn_rb = torch.nn.ModuleList()
        self.ppgn_rb.append(RegularBlock(num_blocks, initial_dim, hidden_channels))
        for i in range(num_rb_layers - 1):
            self.ppgn_rb.append(RegularBlock(num_blocks, hidden_channels, hidden_channels))

        self.ppgn_fc = torch.nn.ModuleList()
        self.ppgn_fc.append(FullyConnected(hidden_channels * 2, hidden_channels))
        for i in range(num_fc_layers - 2):
            self.ppgn_fc.append(FullyConnected(hidden_channels, hidden_channels))
        self.ppgn_fc.append(FullyConnected(hidden_channels, 1, activation_fn=None))
    ...
    def forward(self, data):
        ...
        dense_edge_data = to_dense_adj(
            data.edge_index, data.batch0
        )  # |graphs| * max_nodes * max_nodes * edge_data_dim
        dense_edge_data = torch.unsqueeze(dense_edge_data, -1)
        dense_node_data, mask = to_dense_batch(node_embedding, data.batch0) # |graphs| * max_nodes * d
